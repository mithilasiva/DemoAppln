...
class UsersIT extends Specification {
  ...
  "Users" should {
    ...
    "delete a valid json" in {
      running(FakeApplication()) {
        val request = FakeRequest.apply(DELETE, "/user/A/B").withJsonBody(Json.obj(
          "firstName" -> "A",
          "lastName" -> "B",
          "age" -> 1,
          "active" -> true))
    ...
    "fail delete a non valid json" in {
      running(FakeApplication()) {
        val request = FakeRequest.apply(DELETE, "/user/Jack/London").withJsonBody(Json.obj(
          "firstName" -> "Jack",
          "lastName" -> "London",
          "age" -> 27))
        val response = route(request)
        response.isDefined mustEqual ok
        val result = Await.result(response.get, timeout)
        contentAsString(response.get) mustEqual "invalid json"
        result.header.status mustEqual BAD_REQUEST
      }
